...
function fizzBuzz(max) {
    ...
    const output = [];
    for (let i = 0; i < max; i++) {
        if ((i % 3 === 0 || i % 5 === 0) && i % 15 !== 0) output.push(i);
    }
    return output;
};
...
function isSubString(searchString, subString) {
    ...
    for (let i = 0; i < searchString.length; i++) {
        const endIndex = i + subString.length;
        if (searchString.substring(i, endIndex).toLowerCase() === subString.toLowerCase()) result = true;
    }
    return result;
};
